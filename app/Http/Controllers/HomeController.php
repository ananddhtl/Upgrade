...
class HomeController extends Controller
{
    ...
    public function product(Request $request, $slug)
    {
        ...
        if($detailedProduct->local_shipping){
            $delivered_areas = [];
            foreach(json_decode($detailedProduct->local_shipping_address) as $address){
                $areas = Area::where('city_id',$address->name)->get();
                foreach($areas as $area){
                    array_push($delivered_areas, $area->name);
                }
            }
            $delivered_areas = implode(", ",$delivered_areas);

            $detailedProduct['delivered_areas'] = $delivered_areas;
        }
