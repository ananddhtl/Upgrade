...
class BusinessSettingsController extends Controller
{
     ...
     public function addSliderImage()
    {
        ...
        $home_slider_images = json_decode(get_setting('home_slider_images'));
        array_push($home_slider_images,null);
        $business_settings = BusinessSetting::where('type', 'home_slider_images')->first();
        $business_settings->value = json_encode($home_slider_images);
        $business_settings->save();

        $home_slider_links = json_decode(get_setting('home_slider_links'));
        array_push($home_slider_links,null);
        $business_settings = BusinessSetting::where('type', 'home_slider_links')->first();
        $business_settings->value = json_encode($home_slider_links);
        $business_settings->save();

        $home_slider_type = json_decode(get_setting('home_slider_type'));
        array_push($home_slider_type,null);
        $business_settings = BusinessSetting::where('type', 'home_slider_type')->first();
        $business_settings->value = json_encode($home_slider_type);
        $business_settings->save();

        $home_slider_item_id = json_decode(get_setting('home_slider_item_id'));
        array_push($home_slider_item_id,null);
        $business_settings = BusinessSetting::where('type', 'home_slider_item_id')->first();
        $business_settings->value = json_encode($home_slider_item_id);
        $business_settings->save();
    ...
    public function update(Request $request)
    {
        ...
        foreach ($request->types as $key => $type) {
            ...
            if($type == 'timezone'){
                ...
                if($type=="home_slider_images"){
                    $index = 0;
                    // dd($request);
                    $home_slider_item_idd = $request['home_slider_item_id'];
                    foreach($request->home_slider_type as $home_slider_type){
                        if($home_slider_type=="2"){
                            
                            $home_slider_item_idd[$index] = $request->home_slider_item_id_category[$index];

                            
                            // dd($request->home_slider_item_id[$index],$request->home_slider_item_id_category[$index]);

                        }
                        $index=$index+1;
                    }
                    $request['home_slider_item_id'] = $home_slider_item_idd;
