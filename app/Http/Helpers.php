...
function getShippingCost($carts, $index, $carrier = '')
{
    ...
    if ($shipping_type == 'flat_rate') {
        return get_setting('flat_rate_shipping_cost');
    } elseif ($shipping_type == 'seller_wise_shipping') {
        if ($product->added_by == 'admin') {
            return get_setting('shipping_cost_admin');
        } else {
            return Shop::where('user_id', $product->user_id)->first()->shipping_cost;
        }
        ...
        if($product->local_shipping==='1'){ 
            ...
            if ($city != null) {
                ...
                if ($index !== false) {
                    $local_shipping_address = json_decode($product->local_shipping_address);
                    // dd($local_shipping_address[$index]->price);
                    return $local_shipping_address[$index]->price;
                }
        ...
        foreach ($carrier->carrier_ranges as $carrier_range) {
            if ($itemsWeightOrPrice >= $carrier_range->delimiter1 && $itemsWeightOrPrice < $carrier_range->delimiter2) {
                ...
                return $product->added_by == 'admin' ? ($carrier_price) : ($carrier_price);
            }
        }
        return 0;
    } else {
