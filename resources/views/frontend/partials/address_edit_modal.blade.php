<form class="form-default" role="form" action="{{ route('addresses.update', $address_data->id) }}" method="POST">
    ...
    <div class="p-3">
        ...
        <div class="row">
            ...
            <div class="col-md-10">
                <select class="form-control mb-3 aiz-selectpicker" data-live-search="true" name="area_id" required>
                    @foreach ($cities as $key => $city)
                        <option value="{{ $city->id }}" @if($address_data->area_id == $city->id) selected @endif>
                            {{ $city->name }}
                        </option>
                    @endforeach
                </select>
