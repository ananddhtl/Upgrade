                        ...
                        <div class="shadow-sm bg-white p-4 rounded mb-4">
                            <div class="row gutters-5">
                                @foreach (Auth::user()->addresses as $key => $address)
                                    ...
                                    $city['local_shipping_address_status']=1;
                                    foreach($products as $product){
                                        // dd($product->local_shipping_address);
                                        $index = array_search($city->id, array_column(json_decode($product->local_shipping_address), 'name'));
                                        if ($index === false) {
                                            $city['local_shipping_address_status']=0;
                                            
                                        }
                                        
                                    }
                                    
                                    if($city['local_shipping_address_status']==0){
                                        array_push($inactive_cities, $city->id);
                                    }
                                    @endphp
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label class="aiz-megabox d-block bg-white mb-0">
                                            <input type="radio" name="address_id" value="{{ $address->id }}" @if ($address->set_default)
                                                checked
                                            @endif @if ($city['local_shipping_address_status']==0) disabled @else required @endif>
                                            <span class="d-flex p-3 aiz-megabox-elem">
                                                ...
                                                <span class="flex-grow-1 pl-3 text-left">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span class="fw-600 ml-2">{{ $area->name }}</span>
                                                    </div>
                                                    <div>
                                                    ...
                                                    @if ($city['local_shipping_address_status']==0)
                                                    <span style="color: red;">Address Inactive</span>
                                                    @endif
                                                </span>
                                            </span>
                                        </label>
                            ...
                            @if(count($cities)===count($inactive_cities))
                            <div>
                                @foreach($products as $product)
                                    @php
                                    $shipping_cities=[];
                                    foreach(json_decode($product->local_shipping_address) as $shipping_address){
                                        array_push($shipping_cities, $shipping_address->name);
                                    }
                                    $result = array_intersect($cities, $shipping_cities);
                                    
                                    @endphp
                                    @if(empty($result))

                                    
                                    <p>The product <b>{{$product->name}}</b> is not Delivarable in the above Areas.</p>
                                    @endif
