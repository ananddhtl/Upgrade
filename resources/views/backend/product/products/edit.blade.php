            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div id="category_suggestions"></div><br>
                        ...
                        @endif
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label">{{translate('Is Local Shipping Product?')}}</label>
                            <div class="col-md-8">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input type="checkbox" name="local_shipping" id="local_shipping"  onclick='localShippinghandleClick(this);' @if ($product->local_shipping == 1) checked @endif value="1">
                                    <span></span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-group row" id="local_shipping_address_wrapper">
                            <table class="table table-bordered" id="dynamicTable">  
                                ...
                                @foreach($product->local_shipping_address as $address)
                                <tr>  
                                    <td><select class="form-control" name="local_shipping_address[{{$loop->index}}][name]" id="local_shipping_address['{{$loop->index}}'][name]">
                                        <option value="">{{ translate('Select Branch') }}</option>
                                        

                                        @foreach (\App\Models\City::all() as $city)
                                        <option value="{{ $city->id }}" @if($address->name == $city->id) selected @endif>{{ $city->getTranslation('name') }}</option>
                                        @endforeach
                                        
                                    </select></td>
                                    <td><input type="text" name="local_shipping_address[{{$loop->index}}][price]" placeholder="Enter your Price" class="form-control" value="{{$address->price}}" /></td>  
                                    @if($loop->first)
...
<script type="text/javascript">
    ...
    var i = "<?php echo count($product->local_shipping_address)-1 ?>";
       
    $("#add").click(function(){
   
        ++i;

        $("#dynamicTable").append('<tr><td><select class="form-control" name="local_shipping_address['+i+'][name]"  id="local_shipping_address['+i+'][name]" data-live-search="true"><option value="">{{ translate("Select Branch") }}</option>@foreach (\App\Models\City::all() as $city)<option value="{{ $city->id }}">{{ $city->getTranslation("name") }}</option>@endforeach</select></td><td><input type="text" name="local_shipping_address['+i+'][price]" placeholder="Enter your Price" class="form-control" /></td><td><button type="button" class="btn btn-danger remove-tr">Remove</button></td></tr>');
    ...
    function localShippinghandleClick(checkbox) {
        if(checkbox.checked){
            $("#local_shipping_address_wrapper").show();
        }
        else{
            $("#local_shipping_address_wrapper").hide();
        }
    }
...
<script type="text/javascript">
    ...
    $(document).on("click", ".category_suggestion", function(){
        var selected_value = $("input[name='category_suggestion']:checked").val();
        $("#category_id").val(selected_value).change();
    });
    $(document).ready(function (){
        show_hide_shipping_div();
        if($("#local_shipping").is(":checked")==false){
            $("#local_shipping_address_wrapper").hide();
        }
    });
    ...
    $('input[name="name"]').on('keyup input', function() {
        ...
        var search_text = $('input[name="name"]').val();
        // alert('{{ route('seller.products.category_suggestion') }}');
        ...
        $.ajax({
            ...
            type:"POST",
            url:'{{ route('products.category_suggestion') }}',
            data:{
            ...
            success: function(data) {
                ...
                $('#category_suggestions').html(data);
